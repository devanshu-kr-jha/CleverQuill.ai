...
const CreateNoteDialog = (props: Props) => {
    ...
    const handleSubmit = (e : React.FormEvent<HTMLFormElement>) => {
        e.preventDefault()
        if(input === ''){
            window.alert("Please enter name of your notebook")
            return
        }
        createNotebook.mutate(undefined, {
            onSuccess: ({ note_id }) => {
                console.log('Created new note:', { note_id })
                //hit another endpoint to upload the temp dalle url to storage
                uploadTosSB.mutate(note_id)
